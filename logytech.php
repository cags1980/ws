...
class logytechController {
    ...
    public function CallLogitech(){
        ...
        $this->conect();
        $query = " Execute SP_Despachos_Transporte_Detallado ";
        ...
        foreach ($arraData as $key => $value) {
            //if ($key < 4){
                if ($key == 0){
                    ...
                    $caja_id_despacho = 0;//$value['caja_id_despacho'];
                }
                ...
                if ($value['despacho_id'] == $despacho_id){
                    ...
                    if ($value['caja_id_despacho'] != $caja_id_despacho){
						...
						//$final[$registro]['cod_barra'] = $value['despacho_id'].'-'.$value['caja_id_despacho'];
						...
						$final[$registro]['ubi_direc'] = $value['localidad'];//
                        ...
                        $final[$registro]['productos'][$cnt]['peso_sku'] = $value['peso_caja'];//
                        $final[$registro]['productos'][$cnt]['marca_sku'] = $value['marca'];
                        $final[$registro]['productos'][$cnt]['modelo_sku'] = $value['modelo'];
                        $final[$registro]['productos'][$cnt]['valor_sku'] = $value['mod_escrip'];
                        $final[$registro]['productos'][$cnt]['cantidad_sku'] = $value['qtyunit'];//
                        $cnt++;
                    }else{
                        //$registro++;
                    } 
    ...
    public function apiWS($params){
		...
		var_dump($get);
		echo "/did= " . $params['cod_rastreo'];
		echo "<BR>";
		$did = $params['cod_rastreo'];
		$strdata = print_r($data,1);
		...
		$strget = print_r($get,1);	
		$strget = str_replace('"','',$strget);
		$strget = str_replace("'",'',$strget);
		...
		$strget = str_replace('[','(',$strget);
		$strget = str_replace(']',')',$strget);		
        $query = "insert into Tracking_IX (despacho_id,request_ix,response_ix)
				 VALUES ($did,'$strdata','$strget')";
            $data1 = mssql_query($query);		
        if ($get['error'] < 0){
            //echo $get[0]['msg_error'].'<br>';
            //echo $params['vp_cod_rastreo'].'<br>';z
			$mes = $get['mensaje'];
            $query = " update Despacho_Header set Rta_WS = '$mes'
					 where Despacho_ID = $did";
            $data2 = mssql_query($query);
        }else{
